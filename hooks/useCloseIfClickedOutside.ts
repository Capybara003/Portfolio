    ...
    const checkIfClickedOutside = (e: MouseEvent) => {
      if (isAppCenter) {
        if (
          isModalOpen &&
          modalRef.current &&
          (e.target instanceof HTMLImageElement || e.target instanceof HTMLUListElement || e.target instanceof HTMLButtonElement || e.target instanceof HTMLParagraphElement)
        ) {
          closeModalFunction();
        }
      }
